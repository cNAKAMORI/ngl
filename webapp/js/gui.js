 ...
 * @modified by Chiaki Nakamori <shinobi.div@gmail.com>
 */
...
NGL.MenubarWidget = function (stage, preferences) {
  ...
  if (NGL.functionListUrl && NGL.functionScriptUrl) {
    container.add(new NGL.MenubarFunctionWidget(stage))
  }
...
NGL.MenubarRepresentationWidget = function (stage) {
  ...
  xhr.onload = function (e) {
    ...
    response.sort().forEach(function (name) {
      var option = createOption(name, function () {
        stage.loadScript(NGL.repScriptUrl + name + '.js')
      })
  ...
  return UI.MenubarHelper.createMenuContainer('Representation', optionsPanel)
}

NGL.MenubarFunctionWidget = function (stage) {
  // configure menu contents
  ...
  var xhr = new XMLHttpRequest()
  xhr.open('GET', NGL.functionListUrl)
  xhr.responseType = 'json'
  xhr.onload = function (e) {
    var response = this.response
    if (typeof response === 'string') {
    ...
    response.sort().forEach(function (name) {
      var option = createOption(name, function () {
        stage.loadScript(NGL.functionScriptUrl + name + '.js')
      })
      optionsPanel.add(option)
    })
  }
  ...
  return UI.MenubarHelper.createMenuContainer('Function', optionsPanel)
}
